...
-- SECURITY FIX: Admin password now uses bcrypt instead of MD5
-- Default password is 'admin123' - CHANGE THIS IMMEDIATELY
INSERT INTO `admin` (`id`, `UserName`, `Password`, `updationDate`) VALUES
(1, 'admin', '$2y$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', '2020-05-11 11:18:49');
...
ALTER TABLE `tblusers` ADD INDEX `idx_email` (`EmailId`);
ALTER TABLE `tblbooking` ADD INDEX `idx_package` (`PackageId`);
ALTER TABLE `tblbooking` ADD INDEX `idx_user_email` (`UserEmail`);
ALTER TABLE `tblbooking` ADD INDEX `idx_status` (`status`);
ALTER TABLE `tbltourpackages` ADD INDEX `idx_type` (`PackageType`);
ALTER TABLE `tbltourpackages` ADD INDEX `idx_location` (`PackageLocation`);
...
ALTER TABLE `tblusers` ADD UNIQUE KEY `unique_email` (`EmailId`);

-- Add check constraints for data validation
ALTER TABLE `tblbooking` ADD CONSTRAINT `chk_status` CHECK (`status` IN (0, 1, 2));